...
# sink is needed for expected "pt" argument in the add_systematic callable function, so that all others may be set; it can be set to anything since it's unused
def EGM_scale_and_smear(sink_pt, sink_err, events, year=None, is_correction=True, unc_type=None, restriction=None, is_electron=False, clibhandler=None):
    ...
    if is_correction:
        ...
        if hasattr(events, "GenPart"):
            ...
            smear_correction = smear_and_syst_evaluator("smear", pt_corr, r9, SCeta)
        ...
        if unc_type:
            if unc_type == "Scale":
                ...
                return {"pt":np.concatenate((
                    ak.flatten(egm_object.pt_scale_up[:, None]),
                    ak.flatten(egm_object.pt_scale_down[:, None])), axis=1) * ak.ones_like(egm_object.pt[:, None]),
                        "energyErr": np.concatenate((
                    ak.flatten(egm_object.energyErr_scale_up[:, None]),
                    ak.flatten(egm_object.energyErr_scale_down[:, None])), axis=1) * ak.ones_like(egm_object.pt[:, None])
                        }
            if unc_type == "Smear":
                if not (hasattr(egm_object, "pt_smear_up") and hasattr(egm_object, "pt_smear_down")):
                    raise ValueError(f"EGM Object collection missing pt_smear_down or pt_smear_up fields")
                return {"pt":np.concatenate((
                    ak.flatten(egm_object.pt_smear_up[:, None]),
                    ak.flatten(egm_object.pt_smear_down[:, None])), axis=1) * ak.ones_like(egm_object.pt[:, None]),
                        "energyErr": np.concatenate((
                    ak.flatten(egm_object.energyErr_smear_up[:, None]),
                    ak.flatten(egm_object.energyErr_smear_down[:, None])), axis=1) * ak.ones_like(egm_object.pt[:, None])
                        }
            else:
